...
class AuthController extends Controller
{
    public function registerDevice(RegisterDeviceRequest $request): JsonResponse
    {
        ...
        if ($userDevice->exists() && $userDevice->first()->device_details != $deviceDetails && $userDevice->first()->device_added_at && $userDevice->first()->device_added_at->diffInDays(now()) < User::SINGLE_DEVICE_VALIDITY) {
            return response()->json([
    ...
    public function login(LoginRequest $request): JsonResponse
    {
        ...
        if ($userDevice->exists() && $userDevice->first()->device_details && $userDevice->first()->device_details != $deviceDetails && (!$userDevice->first()->device_added_at || ($userDevice->first()->device_added_at && $userDevice->first()->device_added_at->diffInDays(now()) < User::SINGLE_DEVICE_VALIDITY))) {
            return response()->json([
                'message' => json_encode([
                    'message' => 'already_logged_in',
                    'remaining' => $userDevice->first()?->device_added_at ? ceil(User::SINGLE_DEVICE_VALIDITY - ($userDevice->first()?->device_added_at?->diffInDays(now()))) : 0,
                ]),
            ], 401);
        } elseif ($userDevice->exists() && $userDevice->first()->device_details != $deviceDetails && $userDevice->first()->device_added_at && $userDevice->first()->device_added_at->diffInDays(now()) >= User::SINGLE_DEVICE_VALIDITY) {
            $userDevice->update([
